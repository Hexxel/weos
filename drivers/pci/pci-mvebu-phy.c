...
/* PCIe registers */
#define MSYS_PCIE_LINK_CAPS		0x6c
#define MSYS_PHY_OFFSET				0x1b00
/* System Control registers */
#define MSYS_SOC_CTRL				0x04
#define MSYS_SERDES03_SEL			0x70
#define MSYS_SERDES47_SEL			0x74
...
{
	struct device_node *np = of_find_compatible_node(NULL, NULL,
				 "marvell,msys-system-controller");
	void __iomem *sysctrl = of_iomap(np, 0);
	void __iomem *phybase = pcie->base + MSYS_PHY_OFFSET;
	u32 reg;
	...
	/* Enable PEX */
	reg = readl(sysctrl + MSYS_SOC_CTRL);
	reg |= PCIE0_EN << pcie->port;
	writel(reg, sysctrl + MSYS_SOC_CTRL);

	/* Set SERDES selector */
	reg = readl(sysctrl + MSYS_SERDES03_SEL);
	reg &= ~SERDES_MASK(pcie->port);
	reg |= SERDES(pcie->port, 0x1);
	writel(reg, sysctrl + MSYS_SERDES03_SEL);

	/* BTS #232 - PCIe clock (undocumented) */
	writel(0x00000077, sysctrl + 0x2f0);

	/* Set x1 Link Capability */
	reg = readl(pcie->base + MSYS_PCIE_LINK_CAPS);
	reg &= ~(MAX_LINK_WIDTH_MASK | MAX_LINK_SPEED_MASK);
	reg |= MAX_LINK_WIDTH(0x1) | MAX_LINK_SPEED_5G0;
	writel(reg, pcie->base + MSYS_PCIE_LINK_CAPS);

...
static int fm_eth_set_ethaddr(struct eth_device *edev, const unsigned char *macaddr)
{
	struct fsl_enet_mac *mac;
	struct fm_eth *fm_eth;

	fm_eth = (struct fm_eth *)edev->priv;
	mac = fm_eth->mac;
	mac->set_mac_addr(mac, macaddr);

	return 0;
}

static int fm_eth_get_ethaddr(struct eth_device *edev, u8 adr[6])
{
	return -EOPNOTSUPP;
}
...
static void fm_eth_adjust_link(struct eth_device *dev)
{
	struct fm_eth *fm_eth = dev->priv;
	struct fsl_enet_mac *mac = fm_eth->mac;

	/* set the MAC-PHY mode */
	mac->set_if_mode(mac, fm_eth->enet_if, dev->phydev->speed);
}
static int init_phy(struct eth_device *dev)
{
	...
	if (fm_eth->bus) {
		int ret;
		ret = phy_device_connect(dev, fm_eth->bus, 4 /* HARD CODED. */,
                                         fm_eth_adjust_link, 0, PHY_INTERFACE_MODE_RGMII);

		if (ret)
			return ret;

		fm_eth_adjust_link(dev);
	}

	return 0;
}

static int fm_eth_open(struct eth_device *dev)
{
	struct fm_eth *fm_eth;
	struct fsl_enet_mac *mac;
	...
	mac = fm_eth->mac;
        ...
        fm_load_firmware (0, "/env/firmware/ucode.bin");
	/* enable bmi Rx port */
...
static int fm_eth_probe(struct device_d *dev)
{
	struct eth_device *edev;
	struct fm_eth *fm_eth;
        struct fm_eth_info *info = dev->platform_data;
	int num = info->num;
	...
	if (!edev)
		return -ENOMEM;
	memset(edev, 0, sizeof(struct eth_device));
	...
	mdev = get_device_by_name("memac-mdio0");
	if (mdev == NULL) {
		pr_err("memac-mdio0 was not found\n");
		return -ENODEV;
	}
        fm_eth->regs = dev_get_mem_region(dev, 0);
	...
	fm_eth->rx_port = (void *)&fm_eth->regs->port[info->rx_port_id - 1].fm_bmi;
	fm_eth->tx_port = (void *)&fm_eth->regs->port[info->tx_port_id - 1].fm_bmi;

	/* set the ethernet max receive length */
	fm_eth->max_rx_len = MAX_RXBUF_LEN;

	/* init global mac structure */
	if (!fm_eth_init_mac(fm_eth, fm_eth->regs))
		return -ENODEV;
	...
	devlist[num_controllers++] = edev;

	edev->priv = (void *)fm_eth;
	edev->open = fm_eth_open;
	edev->halt = fm_eth_halt;
	edev->send = fm_eth_send;
	edev->recv = fm_eth_recv;
	edev->set_ethaddr = fm_eth_set_ethaddr;
	edev->get_ethaddr = fm_eth_get_ethaddr;
	edev->parent = dev;

	fm_eth->dev = edev;
	fm_eth->bus = mdev->priv;
...
static struct driver_d fm_eth_driver = {
	.name  = "fm_eth",
	.probe = fm_eth_probe,
};
device_platform_driver(fm_eth_driver);
